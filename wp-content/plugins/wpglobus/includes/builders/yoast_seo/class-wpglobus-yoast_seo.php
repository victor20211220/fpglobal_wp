...
if ( ! class_exists( 'WPGlobus_Yoast_SEO' ) ) :
	...
	class WPGlobus_Yoast_SEO extends WPGlobus_Builder {
		...
		public function __construct() {
			...
			if ( $this->taxonomy ) {
				...
				if ( 'editedtag' !== WPGlobus_WP::get_http_post_parameter( 'action' ) ) {
					 ...
					 * @since 2.7.13
					 */
					add_filter( "option_{$this->wpseo_taxonomy_option_name}", array(
						$this,
						'filter__wpseo_taxonomy_meta',
					), 5, 2 );
				}
		...
		public function filter__wpseo_taxonomy_meta( $option_value, $option_name ) {
			...
			foreach ( $option_value[ $this->taxonomy ] as $tag_ID => $values ) {
				foreach ( $values as $_key => $_value ) {
					if ( in_array( $_key, $_enabled_keys, true ) ) {
		...
		public function on__edit_form( $tag, $taxonomy ) {
			echo '<input type="hidden" name="' . esc_attr( WPGlobus::get_language_meta_key() ) . '" value="' . esc_attr( WPGlobus::Config()->builder->get_language() ) . '">';
		}
	}
endif;
